...
const ConfirmReceiptOfConnectionAgreementStep = ({
  ...
  const handleSave = async () => {
    ...
    await updateData(documentId, {
      ...documentData,
      NgayNhanHoSoThoaThuan: convertDateTimeStringToVnTime(document.getElementById('date-picker-dialog').value),
      TepDinhKemNgayNhanThoaThuan: (getAllUploadFileDialog && getAllUploadFileDialog[getAllUploadFileDialog.length - 1] && getAllUploadFileDialog[getAllUploadFileDialog.length - 1].lastElementChild && getAllUploadFileDialog[getAllUploadFileDialog.length - 1].lastElementChild.firstElementChild)?
  ...
  return (
    <Dialog open={open} onClose={handleClose}>
      <DialogTitle>Nhận thỏa thuận đấu nối</DialogTitle>
      <DialogContent>
        <FormControl fullWidth margin="dense">
          <MDInput mb={2}
            id="date-picker-dialog"
            type="date"
            label="Ngày nhận thoả thuận"
            required
            variant="outlined"
            fullWidth
